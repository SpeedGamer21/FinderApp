...
def scan_drive(path, file_hashes, log, drive_name):
    ...
    for root, dirs, files in os.walk(path):
        ...
        for file in files:
            ...
            scanned_files += 1
            progress_value = scanned_files / total_files * 100
...
def show_duplicates():
    ...
    dup_text.pack(padx=10, pady=10)
    
    for file1, file2 in duplicates_list:
        dup_text.insert(tk.END, f"{file1}  <==>  {file2}\n")
...
progress_label = tk.Label(progress_frame, text="0.00%")
